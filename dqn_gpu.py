...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        self.conv1 = nn.Conv2d(5, 32, kernel_size=7, stride=3)
        ...
        convw = conv2d_size_out(conv2d_size_out(conv2d_size_out(w, 7, 3), 4, 2), 3, 1)
        convh = conv2d_size_out(conv2d_size_out(conv2d_size_out(h, 7, 3), 4, 2), 3, 1)
...
resize = T.Compose([T.ToPILImage(),
                    ...
                    T.Resize((201, 201), interpolation=InterpolationMode.BICUBIC),
                    T.ToTensor()])
...
def random_start(skip_steps=20, m=5):
    env.reset()
